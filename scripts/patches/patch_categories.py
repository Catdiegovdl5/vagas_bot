...
# 1. Update process_mega_location to show categories menu
match_loc = re.search(r'@dp\.callback_query\(F\.data\.startswith\("mega_loc_"\)\)\nasync def process_mega_location.*?await callback\.message\.edit_text\(.*?parse_mode="Markdown"\)', text, re.DOTALL)
if match_loc:
    old_loc = match_loc.group(0)
    new_loc = '''@dp.callback_query(F.data.startswith("mega_loc_"))
async def process_mega_location(callback: CallbackQuery):
    ...
    print("Error: Could not find process_mega_location")

# 2. Add process_mega_category handler right after it
if "async def process_mega_category" not in text:
    new_handler = '''
...
async def process_mega_category(callback: CallbackQuery):
    ...
    for i, term in enumerate(terms):
        try:
            ...
            pass
        await _do_hunt(term, callback.message, callback=callback)
    ...
    try:
        ...
        pass
'''
    text = text.replace('async def process_mega_location', new_handler + '\n\nasync def process_mega_location', 1)

# 3. Clean up handle_free_text
match_text = re.search(r'    if settings\.get\("mega_iniciantes_active"\):.*?return\n', text, re.DOTALL)
if match_text:
    text = text.replace(match_text.group(0), "")

with open('bot.py', 'w', encoding='utf-8') as f:
    f.write(text)

print("Patched categories successfully!")
