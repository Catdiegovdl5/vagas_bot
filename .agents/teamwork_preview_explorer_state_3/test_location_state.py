...
2. Auditoria dos Dicionários de Mapeamento UF (Python & JS):
   Verifica que UF_MAP no Python (bot.py) e #state-select no HTML (static/index.html)
...
def test_app_trigger_location_passing():
    ...
    with patch("bot.is_job_relevant", side_effect=mock_is_job_relevant), \
        ...
        # Como a execução de busca é assíncrona (create_task), aguardamos um breve momento
        import time
        time.sleep(0.3)
...
def test_uf_map_completeness():
    ...
    if match:
        ...
        assert_check(f"UF_MAP possui todas as 27 UFs brasileiras (Faltando: {missing_ufs})", len(found_ufs), 27)
